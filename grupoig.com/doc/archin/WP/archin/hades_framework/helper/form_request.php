<?php
$path = __FILE__;
$pathwp = explode( 'wp-content', $path );
$wp_url = $pathwp[0];

require_once( $wp_url.'/wp-load.php' );

$key = $_POST["key"];
$values = $_POST["values"];
$notify = $_POST["notify_email"];

$forms = get_option("archin_forms");
$cform = '' ;
	  
foreach($forms as $form)
{
	if($key == $form["key"]  )
	{
		$cform = $form; break;
	}
}


$name_value = $cform["name_value"];
$form_element = $cform["form_element"];
$label_values = $cform["label_values"];
$i =0;

$form_data = '';
$captacha_verification  = $cform["captacha_verification"];

if($captacha_verification=="true")
	{
      
	  require(HPATH."/helper/recaptchalib.php");
      $privatekey = get_option("arc_captcha_private_key");;
      $resp = recaptcha_check_answer ($privatekey,
      $_SERVER["REMOTE_ADDR"],
      $_POST["recaptcha_challenge_field"],
      $_POST["recaptcha_response_field"]);

  if (!$resp->is_valid) {
    // What happens when the CAPTCHA was entered incorrectly
    die ("The reCAPTCHA wasn't entered correctly, try it again.");
  }
	}

$response_email = false;

foreach( $form_element as $input )
{
	$label = $label_values[$i];
	$name = $name_value[$i];
	$value = $values[$i];
		
	if($name=="Click to edit name, optional" || trim($name) == "" )
		{
			$name = $input.$i;
		}
	
	if($name=="response_email")
	{
		
		$response_email = $value["value"];
	}
	$form_data = $form_data . "<br/> \n $label :   ".$value["value"]." \n";
	
	$i++;
}


// Create post object
  $my_post = array(
     'post_title' => 'Message',
     'post_content' => $form_data,
     'post_status' => 'publish',
     'post_author' => 1,
     'post_type' => 'message'
  );

// Insert the post into the database
  wp_insert_post( $my_post );
  

if($notify!="none")
{
	
// Email address verification, do not edit.
function isEmail($email) { 
	return(preg_match("/^[-_.[:alnum:]]+@((([[:alnum:]]|[[:alnum:]][[:alnum:]-]*[[:alnum:]])\.)+(ad|ae|aero|af|ag|ai|al|am|an|ao|aq|ar|arpa|as|at|au|aw|az|ba|bb|bd|be|bf|bg|bh|bi|biz|bj|bm|bn|bo|br|bs|bt|bv|bw|by|bz|ca|cc|cd|cf|cg|ch|ci|ck|cl|cm|cn|co|com|coop|cr|cs|cu|cv|cx|cy|cz|de|dj|dk|dm|do|dz|ec|edu|ee|eg|eh|er|es|et|eu|fi|fj|fk|fm|fo|fr|ga|gb|gd|ge|gf|gh|gi|gl|gm|gn|gov|gp|gq|gr|gs|gt|gu|gw|gy|hk|hm|hn|hr|ht|hu|id|ie|il|in|info|int|io|iq|ir|is|it|jm|jo|jp|ke|kg|kh|ki|km|kn|kp|kr|kw|ky|kz|la|lb|lc|li|lk|lr|ls|lt|lu|lv|ly|ma|mc|md|mg|mh|mil|mk|ml|mm|mn|mo|mp|mq|mr|ms|mt|mu|museum|mv|mw|mx|my|mz|na|name|nc|ne|net|nf|ng|ni|nl|no|np|nr|nt|nu|nz|om|org|pa|pe|pf|pg|ph|pk|pl|pm|pn|pr|pro|ps|pt|pw|py|qa|re|ro|ru|rw|sa|sb|sc|sd|se|sg|sh|si|sj|sk|sl|sm|sn|so|sr|st|su|sv|sy|sz|tc|td|tf|tg|th|tj|tk|tm|tn|to|tp|tr|tt|tv|tw|tz|ua|ug|uk|um|us|uy|uz|va|vc|ve|vg|vi|vn|vu|wf|ws|ye|yt|yu|za|zm|zw)$|(([0-9][0-9]?|[0-1][0-9][0-9]|[2][0-4][0-9]|[2][5][0-5])\.){3}([0-9][0-9]?|[0-1][0-9][0-9]|[2][0-4][0-9]|[2][5][0-5]))$/i",$email));		
}

if (!defined("PHP_EOL")) define("PHP_EOL", "\r\n");

$subject  =  "Notification" ; //$_POST['subject'];
$comments = strip_tags($form_data);

if(get_magic_quotes_gpc()) {
	$comments = stripslashes($comments);
}

$address = $notify;
$e_subject = 'You\'ve have a message';

$e_body = "This is a notify mail." . PHP_EOL . PHP_EOL;
$e_content = "\"$comments\"" . PHP_EOL . PHP_EOL;
		
$msg = wordwrap( $e_body . $e_content . $e_reply, 70 );

$headers = "From: Autogen@noreply.com" . PHP_EOL;
$headers .= "Reply-To: Autogen@noreply.com" . PHP_EOL;
$headers .= "MIME-Version: 1.0" . PHP_EOL;
$headers .= "Content-type: text/plain; charset=utf-8" . PHP_EOL;
$headers .= "Content-Transfer-Encoding: quoted-printable" . PHP_EOL;

if($response_email)
{
	
	mail($response_email, "Confirmation","Hi we have received your mail, will be in touch shortly", $headers);
}

if(mail($address, $e_subject, $msg, $headers)) {

	// Email has sent successfully, echo a success page.
	
	echo "success";
		 
} else {

	echo 'ERROR!';

}
}